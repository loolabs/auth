...
export type RedisDeleteEntitySuccess = true
  ...
  async saveEntity(
    ...
    return new Promise((resolve) => {
      const callback = (err: Error | null) => {
        if (err) {
          resolve(Result.err(new AppError.UnexpectedError(`Redis save operation failed. ${err}`)))
        } else {
  ...
  async removeEntity(redisEntity: RedisEntity): Promise<RedisDeleteEntityResponse> {
    return new Promise((resolve) => {
      RedisClient().del(redisEntity.getEntityKey(), (err) => {
        if (err) {
          resolve(Result.err(new AppError.UnexpectedError(`Redis delete operation failed. ${err}`)))
        } else {
          resolve(Result.ok(true))
        }
      })
