...
export type GetUserUseCaseError = GetUserErrors.GetUserByIdFailedError | AppError.UnexpectedError

export type GetUserUseCaseResponse = Result<User, GetUserUseCaseError>

export class GetUserUseCase implements UseCaseWithDTO<GetUserDTO, GetUserUseCaseResponse> {
  ...
  async execute(dto: GetUserDTO): Promise<GetUserUseCaseResponse> {
    ...
    if (userById.isErr()) return Result.err(new GetUserErrors.GetUserByIdFailedError(userId))
